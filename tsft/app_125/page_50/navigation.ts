...
export async function acessoAPP125_50(page: Page, context: BrowserContext) {
    await login(page);

    // Pesquisa pagina
    const buttonFinanceiro = await page.locator('div').filter({ hasText: /^Entidades$/ });
    ...
    // 4. Validação básica
    await expect(newPage).toHaveTitle('Entidades');

    // acessa pagina
    await newPage.getByRole('link', { name: ' Pessoa' }).click();
